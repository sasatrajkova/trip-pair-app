...
public class LocationRepository : ILocationRepository
{
    ...
    {
        var location = await _db.Locations.Include(l => l.LocationMonths).ThenInclude(lm => lm.Month)
    ...
    {
        var location = await _db.Locations.Where(l => l.Name.ToLower().Trim() == locationName.ToLower().Trim())
            .FirstOrDefaultAsync();
        return location;
    }
    ...
    {
        var locationMonths = await _db.LocationMonths.Where(lm => lm.LocationId == locationId).Select(lm => lm.Month)
            .ToListAsync();
    ...
    public async Task<Location?> UpdateLocation(int id, Location locationToUpdate)
    {
        ...
        foreach (var existingLocationMonth in existingLocationMonths) _db.LocationMonths.Remove(existingLocationMonth);
    ...
    public async Task<Location?> DeleteLocation(int id)
    {
        ...
        var locationMonthsToDelete =
            await _db.LocationMonths.Where(lm => lm.LocationId == locationToDelete.Id).ToListAsync();
        foreach (var locationMonth in locationMonthsToDelete) _db.LocationMonths.Remove(locationMonth);
